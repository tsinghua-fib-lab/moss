...
class ParallelManager(Engine):
    ...
    def __init__(self, engines: List[ParallelEngine]):
        if not all(isinstance(e, ParallelEngine) for e in engines):
            raise NotImplementedError("ParallelManager can only be used with ParallelEngine!")
...
def _proxy_init(self, *args, **kwds):
    ...
    self._q_in = Queue()
    self._q_out = Queue()
    self.__proc = threading.Thread(target=_instance, args=(args, kwds, self._q_in, self._q_out), daemon=True)
    self.__proc.start()
...
def _init():
    ...
    for name in dir(Engine):
        ...
        if isinstance(f, property):
            ...
            setattr(ParallelManager, name, property(partial(_proxy_property_batch, name)))
        elif callable(f):
